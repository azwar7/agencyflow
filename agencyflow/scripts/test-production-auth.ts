...
import { requireAuth, requireRole, requireWorkspaceOwnership } from '../src/lib/authorization';
...
async function runTests() {
  ...
  try {
    ...
    // -------------------------------------------------------------
    // TEST SUITE 3: Database Session Creation & Token Hashing
    // -------------------------------------------------------------
    console.log('\n--- TEST SUITE 3: Database Sessions ---');
    ...
    // -------------------------------------------------------------
    // TEST SUITE 7: Server-Side RBAC (Role-Based Access Control)
    // -------------------------------------------------------------
    console.log('\n--- TEST SUITE 7: Server-Side RBAC ---');
    ...
    try {
      ...
      roleCheckForbidden = e.message.includes('Forbidden') || e.message.includes('not authorized');
    }
    assert(roleCheckForbidden, 'SALES_REP is blocked from OWNER/ADMIN team management operations');
